...
namespace Fluxo.Core
{
    public static class ArgsProcessor
    {
        ...
        private static bool IsExtensionRegistration(IEnumerable<string> args)
        {
            if (args.Count() > 0)
            {
                return args.Where(arg => arg.StartsWith("fluxo-app:chrome-extension://")).Count() == 1;
            }
            return false;
        }
...
//        {
//            ExtensionRegistrationHelper.AddExtension(args.Where(arg => arg.StartsWith("fluxo-app:chrome-extension://")).First().Substring("fluxo-app:".Length));
//#if WINDOWS
