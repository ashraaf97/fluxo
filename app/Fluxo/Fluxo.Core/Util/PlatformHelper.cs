...
namespace Fluxo.Core.Util
{
    public static class PlatformHelper
    {
        ...
        {
            var firstRunFile = Path.Combine(Config.AppDir, "fluxo-" + AppInfo.APP_VERSION + ".first-run");
            ...
            try
            {
                ...
                if (os == PlatformID.Win32NT)
                {
                    ...
                    if (hkcuRun != null)
                    {
                        if (enable)
                        {
                            var fluxoExe = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "fluxo-app.exe");
                            hkcuRun.SetValue("Fluxo", $"\"{fluxoExe}\" --background");
                        }
                        else
                        {
                            hkcuRun.DeleteValue("Fluxo", false);
                        }
                    }
                    return true;
                }
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    ...
                    var desktopFile = Path.Combine(autoStartDir, "fluxo-app.desktop");
        ...
        {
            var appPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "fluxo-app");
            var iconPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "fluxo-logo.svg");
        ...
        //         String s2 = FluxoUtils.getJarFile().getAbsolutePath();
        ...
        public static bool IsAutoStartEnabled()
        {
            try
            {
                ...
                if (os == PlatformID.Win32NT)
                {
                    ...
                    if (hkcuRun != null)
                    {
                        var command = (string)hkcuRun.GetValue("Fluxo");
                        var path = FileHelper.GetFileNameFromQuote(command);
                        return !string.IsNullOrEmpty(path) &&
                            path == Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "fluxo-app.exe");
                    }
                }
#if NET5_0_OR_GREATER
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    ...
                    var file = Path.Combine(autoStartDir, "fluxo-app.desktop");
                    if (!File.Exists(file))
                    {
                        return false;
                    }
                    var text = File.ReadAllText(file);
                    return text.Contains(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "fluxo-app"));
                }
#endif
            }
            catch (Exception ex)
