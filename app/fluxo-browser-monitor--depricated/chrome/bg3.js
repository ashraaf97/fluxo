...
(function () {
    ...
    var monitoring = true;
    var debug = true;
    var fluxoHost = "http://127.0.0.1:9614";
    var disabled = false;
    var lastIcon;
    var lastPopup;
    var videoList = [];
    ...
    function processRequest(request, response) {
        if (shouldInterceptFile(request.request, response)) {
            ...
            sendToFluxo(request.request, response, file, false);
    ...
    function sendToFluxo(request, response, file, video, requestBody) {
        log("sending to fluxo: " + response.url);
        if (hasNativeMessagingHost) {
            ...
            chrome.cookies.getAll({ "url": response.url }, function (cookies) {
                ...
                xhr.open('POST', fluxoHost + (video ? "/video" : "/download"), true);
                xhr.send(data);
            });
        }
    };
    ...
    function sendRecUrl(urls, index, data) {
        if (index > 0 && index == urls.length - 1) {
            ...
            if (hasNativeMessagingHost) {
                ...
                var xhr = new XMLHttpRequest();
                xhr.open('POST', fluxoHost + "/links", true);
                xhr.send(text);
            }
            return;
        }
    ...
    function sendUrlsToFluxo(urls) {
        if (urls && urls.length > 0) {
            sendRecUrl(urls, 0, []);
        }
    };

    function sendUrlToFluxo(url) {
        log("sending to fluxo: " + url);
        if (hasNativeMessagingHost) {
            chrome.cookies.getAll({ "url": url }, function (cookies) {
                var cookieDict = {};
                cookies.forEach(cookie => {
            ...
            chrome.cookies.getAll({ "url": url }, function (cookies) {
                ...
                log(data);
                var xhr = new XMLHttpRequest();
                xhr.open('POST', fluxoHost + "/download", true);
                xhr.send(data);
            });
        }
    };

    function sendImageToFluxo(info, tab) {
        if (info.mediaType) {
    ...
    function sendLinkToFluxo(info, tab) {
            ...
            if (request.request.tabId != -1) {
                ...
                window.setTimeout(() => {
                    chrome.tabs.get(
                        ...
                        function (tab) {
                            sendToFluxo(request.request, response, tab.title, true, requestBody);
                        }
                    );
                }, 2000);
            } else {
                sendToFluxo(request.request, response, null, true, requestBody);
            }
        }
    };
    ...
    function syncFluxo() {
        ...
        xhr.open('GET', fluxoHost + "/sync", true);
        xhr.send(null);
    };
    ...
    function onHeadersReceived(response) {
        ...
        if (!(isFluxoUp && monitoring && !disabled)) {
            return;
        }
        if (response.statusCode !== 200 && response.statusCode !== 206) {
            return;
        }
        if (!response.url || response.url.startsWith(fluxoHost)) {
            return;
        }
    ...
    function connectToNativeMessagingHost() {
        return new Promise((resolve, reject) => {
            try {
                log("Connecting to native messaging host: fluxo_chrome.native_host");
                port = chrome.runtime.connectNative("fluxo_chrome.native_host");
    ...
    function initSelf() {
            ...
            function (request, sender, sendResponse) {
                if (request.type === "links") {
                    ...
                    if (hasNativeMessagingHost) {
                        ...
                        var xhr = new XMLHttpRequest();
                        xhr.open('POST', fluxoHost + "/item", true);
                    ...
                    if (hasNativeMessagingHost) {
                        ...
                        var xhr = new XMLHttpRequest();
                        xhr.open('GET', fluxoHost + "/clear", true);
                        xhr.send();
                    }
                }
        ...
        chrome.contextMenus.create({
            ...
            onclick: sendLinkToFluxo,
        });

        chrome.contextMenus.create({
            title: "Download Image with Fluxo",
            contexts: ["image"],
            onclick: sendImageToFluxo,
        });
        ...
        */
        connectToNativeMessagingHost().catch(err => {
            //play nice with older Fluxo versions
            setInterval(function () { syncFluxo(); }, 5000);
        });
    };
