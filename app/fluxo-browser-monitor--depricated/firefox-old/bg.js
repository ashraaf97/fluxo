(function () {
    ...
    var processRequest = function (request, response) {
        if (shouldInterceptFile(request, response)) {
            ...
            sendToFluxo(request, response, file, false);
    ...
    var sendToFluxo = function (request, response, file, video) {
    ...
    var sendUrlsToFluxo = function (urls) {
        sendRecUrl(urls, 0, "");
    };

    var sendUrlToFluxo = function (url) {
    ...
    var sendImageToFluxo = function (info, tab) {
        if (info.mediaType) {
    ...
    var sendLinkToFluxo = function (info, tab) {
            ...
            if (request.tabId != -1) {
                chrome.tabs.get
                    (
                        request.tabId,
                        function (tab) {
                            sendToFluxo(request, response, tab.title, true);
                        }
                    );
            } else {
                sendToFluxo(request, response, null, true);
            }
        }
    };
    ...
    var initSelf = function () {
                ...
                function (response) {
                    ...
                    if (requests) {
                        if (requests.length == 1) {
                            if (!(response.url + "").startsWith(fluxoHost)) {
        ...
        //check Fluxo if is running and enable monitoring
        ...
        */
        port = browser.runtime.connectNative("fluxoff.native_host");
